...
private:
    ...
    bool askQuestion() {
        ...
        int a=rand()%maxVal+1, b=rand()%maxVal+1;
        int op=rand()%4;
        char ops[]="+−×÷"; // display only
        int answer;

        if (op==0) { answer=a+b; cout<<"\n"<<a<<" + "<<b<<" = ? "; }
        else if (op==1) { answer=a-b; cout<<"\n"<<a<<" - "<<b<<" = ? "; }
        else if (op==2) { a=rand()%12+1; b=rand()%12+1; answer=a*b; cout<<"\n"<<a<<" x "<<b<<" = ? "; }
        else { b=rand()%9+1; a=b*(rand()%10+1); answer=a/b; cout<<"\n"<<a<<" / "<<b<<" = ? "; }

        int userAns;
        if (!(cin>>userAns)) { cin.clear(); cin.ignore(1000,'\n'); return false; }
        if (userAns==-999) { answered=999; return false; }
        if (userAns==answer) { cout<<"  Correct! +1\n"; return true; }
